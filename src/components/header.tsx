...
export default function Header() {
  ...
  return (
    <section className="wrapper fixed top-0 inset-x-0 z-50 bg-backround">
      <div className="container h-24 flex justify-between items-center px-4 md:px-16">
        <Link to="/">
          <h1 className="text-xl md:text-2xl">ASCEND</h1>
        </Link>

        <div className="hidden md:flex items-center">
          <Link to="/" hash="overview">
            <Button text="about" />
          </Link>
          <Link to="/" hash="speakers">
            <Button text="speakers" />
          </Link>
          <Link to="/" hash="schedule">
            <Button text="schedule" />
          </Link>
          <Link to="/" hash="location">
      ...
      >
        <Link to="/" hash="overview" onClick={() => setOpen(false)}>
          <Button text="about" />
        </Link>
        <Link to="/" hash="speakers" onClick={() => setOpen(false)}>
          <Button text="speakers" />
        </Link>
        <Link to="/" hash="schedule" onClick={() => setOpen(false)}>
          <Button text="schedule" />
        </Link>
        <Link to="/" hash="location" onClick={() => setOpen(false)}>
          <Button text="location" />
        </Link>
        <Link to="/" hash="passes" onClick={() => setOpen(false)}>
          <Button text="get tickets" variant="button" />
        </Link>
      </div>
    </section>
